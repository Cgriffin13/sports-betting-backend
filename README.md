...
`/opportunities` remains the pricing-only baseline and returns no stake. For current NCAAF spreads/totals, `ncaaf-empirical-cross-line-v1` preserves exact coherent sides inside each book, infers a robust fair line across different supported-book main lines, and evaluates every executable line with explicit empirical win/push/loss mass. Moneyline remains proportional no-vig plus unweighted-median consensus. The Phase 6 recommendation route then applies unchanged 0.75pp edge / 1.5% EV gates, quarter-Kelly risk budgeting, robust expected-log-growth ordering, CORE/OPPORTUNISTIC classification, Top-N/PASS behavior, and human approval. Because no separately risk-budgeted longshot sleeve exists, prices above `+500` remain calculable diagnostics but cannot become ordinary actionable recommendations; this safety guardrail does not alter fair value or EV. See [`NCAAF_MARKET_VALUE_CALIBRATION.md`](docs/NCAAF_MARKET_VALUE_CALIBRATION.md), [`PORTFOLIO_RISK_AND_RECOMMENDATIONS.md`](docs/PORTFOLIO_RISK_AND_RECOMMENDATIONS.md), and the bounded [`LIVE_PRICING_METHODOLOGY_AUDIT.md`](docs/LIVE_PRICING_METHODOLOGY_AUDIT.md).
...
The proprietary probability engine remains research-only and is not connected to `/opportunities`. Market consensus remains the production fair-probability source. The current NCAAF market-only path now uses its separately validated empirical market-residual lattice to assign explicit push probability at integer spread/total lines; it does not import or substitute a rejected football model. See [`NCAAF_PROBABILITY_CALIBRATION_REPORT.md`](docs/NCAAF_PROBABILITY_CALIBRATION_REPORT.md) and [`NCAAF_MARKET_VALUE_CALIBRATION.md`](docs/NCAAF_MARKET_VALUE_CALIBRATION.md).
