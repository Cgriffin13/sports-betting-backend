...
> Phase 5B-6 implementation note (2026-08-30): bounded reconstructed preseason/personnel research is complete. Recruiting/talent and a regularized preseason adjustment to the chronological power rating advance for further offline margin research; full preseason Ridge total and the common-era transfer family are rejected. Limited probability checks did not displace the existing margin/total probability benchmarks. See [`NCAAF_PRESEASON_MODEL_REPORT.md`](NCAAF_PRESEASON_MODEL_REPORT.md).
