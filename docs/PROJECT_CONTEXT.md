...
> POLARIS also exposes a research-only Watchlist derived from the same backend candidate evaluation. It shows positive-edge markets that survived baseline pricing but missed unchanged production gates. Watchlist rows carry no stake, cannot be approved, never enter the ledger or parlay optimizer, and do not count as qualified recommendations.
