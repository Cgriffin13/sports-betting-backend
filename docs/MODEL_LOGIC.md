...
The approved first operational workflow is one game-day-morning run before the day's first NCAAF kickoff. Its exact fixed-time or first-kickoff-relative convention remains gated on historical coverage. The 60-minute and 24-hour cutoffs are separate research horizons and must never be combined or substituted. A historical row may use only source and feature records available at its cutoff, including observation, source-snapshot/effective, and ingestion semantics appropriate to its declared availability mode. Closing lines, final availability, realized weather, postgame corrections, and future opponent results cannot leak backward.
