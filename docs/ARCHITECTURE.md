...
The dashboard now has one explicit write-like market workflow: `POST /dashboard/portfolio/{portfolio_id}/refresh-markets`. The browser calls the Cloudflare Pages Function; the BFF injects `APP_API_KEY`; FastAPI acquires a process-local nonblocking refresh guard; the provider adapter performs its existing bounded retry/cache policy; `MarketIngestionService` persists raw and normalized state transactionally; `RecommendationService` evaluates each upcoming slate at the post-commit `ingestion_completed_at` cutoff; and the client invalidates and rereads PostgreSQL-backed dashboard queries. Browser reads never call the provider. The response keeps request start, provider retrieval, ingestion completion, and decision cutoff separate. Historical replay continues to use its caller-supplied cutoff.
...
The pricing read path separates current-snapshot freshness from provider quote age. SQL latest-state selection prioritizes `market_snapshots.requested_at`, projects only normalized scalar fields, and enforces `observed_at`, `requested_at`, and `ingested_at` cutoffs. An explicit live refresh supplies a decision cutoff at or after its newly persisted observations; offline/historical analysis retains the requested historical cutoff and therefore cannot see later ingestion. The domain layer applies the snapshot policy while reporting provider quote-age min/median/p90/max and a distinct pathological-age rejection. The multi-slate diagnostic exposes the freshest/latest snapshot-age gauge rather than summing ages or selecting the oldest slate's gauge.
