...
class CanonicalEvent(Base):
    ...
    venue_id: Mapped[UUID | None] = mapped_column(ForeignKey("canonical_venues.id", ondelete="RESTRICT"))
    season: Mapped[int | None] = mapped_column(Integer)
    week: Mapped[int | None] = mapped_column(Integer)
    season_type: Mapped[str | None] = mapped_column(String(32))
    neutral_site: Mapped[bool | None] = mapped_column(Boolean)
    schedule_revision: Mapped[str | None] = mapped_column(String(128))
    schedule_provenance: Mapped[dict[str, Any] | None] = mapped_column(JSON_DOCUMENT)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=utc_now)
