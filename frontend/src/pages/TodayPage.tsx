...
export function TodayPage() {
  ...
  const gamesAnalyzed = new Set(data.recommendations.map((item) => item.event_id).filter(Boolean)).size || data.movement.events.length;
  return <div className="page-stack">
    ...
    <div className="metric-grid five"><Metric label="Portfolio equity" value={currency(data.portfolio.equity)} detail={`${currency(data.portfolio.cash)} available cash`} icon={WalletCards} /><Metric label="Total exposure" value={currency(data.risk.reserved_exposure)} detail={`${percent(dailyExposure)} of equity`} icon={Shield} tone={dailyExposure > dailyLimit * 0.8 ? "warning" : "default"} /><Metric label="Open risk" value={currency(data.portfolio.open_exposure)} detail={`${data.portfolio.bets.filter((bet) => bet.status === "open").length} open positions`} icon={Banknote} /><Metric label="Today's P&L" value={currency(data.stats.net_pnl)} detail={`${percent(Number(data.stats.overall.roi ?? 0))} portfolio ROI`} icon={TrendingUp} tone={data.stats.net_pnl >= 0 ? "positive" : "negative"} /><Metric label="Upcoming slate" value={`${gamesAnalyzed} games`} detail={`${data.recommendations.length} qualified · ${core.length} core`} icon={ScanSearch} /></div>
