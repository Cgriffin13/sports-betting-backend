...
import { QualifiedOpportunitiesTable } from "../components/QualifiedOpportunitiesTable";
import { WatchlistTable } from "../components/WatchlistTable";
import { ErrorPage, LoadingPage, Metric, PageHeader, Panel, StatusBadge } from "../components/Primitives";
import { demoData } from "../data/demo";
import { useDashboard, useMarketRefresh } from "../hooks/useDashboard";
...
export function TodayPage() {
  ...
  const gamesAnalyzed = data.watchlist.upcoming_games_analyzed;
  return <div className="page-stack">
    <PageHeader eyebrow="NCAAF · Today" title="Portfolio decision desk" description="Qualified market-consensus opportunities, upcoming slate coverage, exposure controls, and approval-ready paper positions." actions={<div className="page-actions"><StatusBadge tone={data.system.market_status === "FRESH" ? "positive" : data.system.market_status === "ERROR" ? "negative" : "warning"}>{data.system.market_status}</StatusBadge><button className="button primary refresh-markets" disabled={refresh.isPending} onClick={() => refresh.mutate()}><RefreshCw className={refresh.isPending ? "spin" : ""} size={16} />{refresh.isPending ? "Refreshing markets…" : "Refresh Markets"}</button></div>} />
    {refresh.isSuccess && <div className="refresh-result positive"><CircleCheckBig size={17} /><div><strong>Markets refreshed</strong><span>{refresh.data.upcoming_events} upcoming games · {refresh.data.observations_created} stored observations · dashboard state reloaded</span></div></div>}
    {refresh.isError && <div className="warning-banner"><AlertTriangle size={17} /><div><strong>Market refresh failed</strong><span>{refresh.error.message}. Existing stored recommendations remain unchanged.</span></div></div>}
    {!refresh.isError && data.system.market_status === "ERROR" && <div className="warning-banner"><AlertTriangle size={17} /><div><strong>Latest provider refresh failed</strong><span>{data.system.last_provider_error || data.system.market_status_reason}. Previously persisted market state remains available with its original timestamp.</span></div></div>}
    {data.system.stale && <div className="warning-banner"><AlertTriangle size={17} /><div><strong>Stored market data is stale</strong><span>{data.system.market_status_reason}. Use Refresh Markets before approving a position; navigation and page reloads never call the provider.</span></div></div>}
    {data.watchlist.pricing_pipeline_status === "DEGRADED" && <div className="warning-banner"><AlertTriangle size={17} /><div><strong>Pricing pipeline degraded</strong><span>{titleCase(data.watchlist.pricing_pipeline_status_reason || "candidate pipeline collapsed before pricing")}. This is an integrity warning, not a successful PASS.</span></div></div>}
    <div className="metric-grid five"><Metric label="Portfolio equity" value={currency(data.portfolio.equity)} detail={`${currency(data.portfolio.cash)} available cash`} icon={WalletCards} /><Metric label="Total exposure" value={currency(data.risk.reserved_exposure)} detail={`${percent(dailyExposure)} of equity`} icon={Shield} tone={dailyExposure > dailyLimit * 0.8 ? "warning" : "default"} /><Metric label="Open risk" value={currency(data.portfolio.open_exposure)} detail={`${data.portfolio.bets.filter((bet) => bet.status === "open").length} open positions`} icon={Banknote} /><Metric label="Today's P&L" value={currency(data.stats.net_pnl)} detail={`${percent(Number(data.stats.overall.roi ?? 0))} portfolio ROI`} icon={TrendingUp} tone={data.stats.net_pnl >= 0 ? "positive" : "negative"} /><Metric label="Upcoming slate" value={`${gamesAnalyzed} games`} detail={`${data.watchlist.qualified_recommendations} qualified · ${data.watchlist.actionable_recommendations} actionable · ${data.watchlist.watchlist_count} watchlist`} icon={ScanSearch} /></div>
    ...
    <Panel title="Core picks" eyebrow="Highest risk-adjusted quality" action={<StatusBadge tone="positive">{core.length} actionable</StatusBadge>}><PicksTable recommendations={core} label="CORE" /></Panel>
    <Panel title="Qualified opportunities" eyebrow="Passed pricing · blocked by portfolio controls" action={<StatusBadge tone="warning">{data.watchlist.qualified_opportunities.length} non-actionable</StatusBadge>}><p className="panel-description qualified-opportunity-note">These candidates passed every pricing gate, but no Recommendation row or approval action exists because portfolio allocation rejected the stake.</p><QualifiedOpportunitiesTable items={data.watchlist.qualified_opportunities} /></Panel>
    <Panel title="On the radar" eyebrow="Research only · near qualification" className="watchlist-preview" action={<Link className="text-link" to="/watchlist">View full watchlist</Link>}><WatchlistTable items={data.watchlist.items} limit={5} /></Panel>
    ...
    <Panel title="Opportunistic picks" eyebrow="Positive EV · smaller allocation" action={<StatusBadge tone="info">{opportunistic.length} actionable</StatusBadge>}><PicksTable recommendations={opportunistic} label="OPPORTUNISTIC" /></Panel>
...
function PassPanel({ data }: { data: NonNullable<ReturnType<typeof useDashboard>["data"]> }) {
  if (data.watchlist.pricing_pipeline_status === "DEGRADED") return <Panel title="Pricing pipeline degraded" eyebrow="Integrity warning"><div className="pass-explanation"><AlertTriangle /><div><h3>Decision unavailable</h3><p>Market observations were present, but the pipeline collapsed before a healthy candidate set could be calculated. Do not interpret this as capital-preserving PASS.</p><ul><li>{titleCase(data.watchlist.pricing_pipeline_status_reason || "unknown pipeline integrity failure")}</li></ul></div></div></Panel>;
...
function useScanDelta(ids: string[]) {
  ...
  useEffect(() => {
    const key = "polaris:last-recommendation-ids";
